...
from utils.train_eval_utils_reject import train_one_epoch_reject, evaluate_reject
...
def main(args):
    ...
    print("using {} HRRP datas for training, {} HRRP data for validation.".format(train_size,
                                                                                  val_size))
    # 如果存在预训练权重则载入
    model = resnet18(args.num_classes, dimension_reduction=True).to(device)   # 修改模型
    ...
    for epoch in range(args.epochs):
        # train
        train_loss, train_acc = train_one_epoch_reject(model=model,
                                                optimizer=optimizer,
        ...
        # validate
        val_loss, val_acc = evaluate_reject(model=model,
        ...
        #     tb_writer.add_histogram(tag=name + '_data', values=param.data.numpy(), global_step=epoch)
